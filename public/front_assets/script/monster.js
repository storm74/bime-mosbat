...
$(document).ready(function() { "use strict";
  ...
  function updateHash(){
	...
	$('.mainMenu a[href^="#'+isHash+'"').addClass('active');
	}
  ...
  */ 
  ...
  //Change slide
  window.changeSlide = function(n,past){
	...
	if ($('body .slide').eq(n-1).hasClass('portfolio')) {
		console.log (past);
		if ($('body .slide').eq(past-1).hasClass('portfolio')) {
  ...
  $('.nextSlide').on('click', function(){
    window.changeSlide('increase',window.stage);
  });
  
  $('.prevSlide').on('click', function(){
    window.changeSlide('decrease',window.stage);
  });
  
  $('.toFirstSlide').on('click', function(){
    window.changeSlide(1,window.stage);
    if (history.pushState) {
      window.history.pushState("", "", location.href.split('#')[0]); 
    } else {
      window.location.hash = "";
  ...
  $('.toLastSlide').on('click', function(){
    window.changeSlide(window.stages,window.stage);
    if (history.pushState) {
      window.history.pushState("", "", location.href.split('#')[0]); 
    } else {
      window.location.hash = "";
  ...
  $('[class*="toSlide-"]').on('click', function(){
    ...
    window.changeSlide(num,window.stage);
    if (window.isMobile){
      hideSidebar();
    }
  });
  ...
  function updateScroll(){
    ...
    $('.slide').each(function(index, element) {
      ...
      //Parallax motion
      if ($element.hasClass('parallaxMotion') && !window.isMobile){
                  
        $element.find('.parallaxElement').each(function(){

          var $el = $(this),
              styles = $el.attr('style'),
              posY = $el.attr('style','').offset().top,
              elHeight = $el.height(),
              scale = (((scrollTop + windowHeight) - posY) / (windowHeight + elHeight) - 0.5) * 2;

          if (scale > -1.5 && scale < 1.5) {

            $el.attr('style',styles);

            var distance = (parseInt($el.data('distance'))) ? parseInt($el.data('distance')) : 50,
            shift =  scale * distance;

            $el.css('-webkit-transform',"translate3d(0, " + shift + "px, 0)").css('transform',"translate3d(0, " + shift + "px, 0)");
          }
  ...
  function isElementInView(element) {
    ...
    if ($element.height() >= $(window).height() + 0.5) {
      return (pageBottom >= elementTop + $element.height() + 0.5);
    } else {
  ...
  /*
    ...
    if ($('body').hasClass('disableKeyNavigation') || e.target.nodeName.toLowerCase() == 'input' || e.target.nodeName.toLowerCase() == 'textarea') {
      return;
		...
		if (e.keyCode === 37){
      e.preventDefault();
			if (window.horizontalMode){ window.changeSlide('decrease',window.stage); }
		}
  ...
  */
  ...
  if ($(navigation).length > 0) {
    ...
    $('.navigation li').on("click touchend", function(){
      ...
      window.changeSlide(realIndex + 1,window.stage);
    });
    
    if (!$('.side').hasClass('compact')){
      //Collapse sidemenu to compact
      $(window).on('load resize ready',function(){
        var containerWidth = $(window).height() - $(window).width()*0.06 ,
  ...
  if ($controller.length > 0) {
    ...
    $controller.on('click', controllerSelector, function(){
      ...
      if (!$controllerElement.hasClass('selected')){
        ...
        $slider.find('.selected').removeClass('selected').addClass('hide').one('webkitTransitionEnd otransitionend oTransitionEnd msTransitionEnd transitionend', function() {
          $(this).removeClass('hide');
        });
  ...
  $('.dropdownTrigger').on('click', function(){
    ...
    if ( elementPosition.toLowerCase().indexOf("bottom") >= 0 ) {
    ...
    $html.addClass('dropdownShown dropdown_' + shareID);
